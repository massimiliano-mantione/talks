...
const images = {
  ...
  codemotion: require('../assets/codemotion_logo.png'),
...
const slideTrap = (children) => {
  return (
    <Slide
      transition={['zoom', 'slide']}
      bgImage={images.speedtrap.replace('/', '')}
      bgDarken={0.75}
      textColor="primary"
    >{children}</Slide>)
}
...
const logo = () => {
  return image(images.codemotion, '30vw')
}
const talkQrCode = () => {
...
const slides = () => {
  return [
    ...
    slideZoom([
      logo(),
      titleNoCaps('Milano, Oct 11 2017', false, 6),
    ...
    slide([
      ...
      lineBold('Different question, please')
    ]),
...
var a = [77, 88, 0.5, true];`)
    ...
    slide([
      ...
      line('"Full" compiler (fullGen) generated good code for any JavaScript'),
      line('Optimizing compiler (crankshaft) produced great code for most JavaScript')
    ]),

    slide([
      title('More engines!'),
      lineBold('V8 gained two more engines'),
      lineEm('and lost the previous ones'),
    ...
    slide([
      title('Compilation Pipeline', false, 6),
      image(images.v8newPipeline, '80vw')
    ]),

    slide([
      title('Ignition'),
      line('An interpreter that starts executing code ASAP'),
      line('Uses an internal bytecode representation'),
      lineEm('Initially assumes nothing about types')
    ]),
    ...
    slide([
      ...
      line('ICs get better at runtime'),
      line('Type information is used by turbofan')
    ]),

    slide([
      title('Javascript Code'),
      codeBlock(`// Consider this.primes
this.isPrimeDivisible = function(candidate) {
  for (var i = 1; i <= this.prime_count; ++i) {
    if (candidate % this.primes[i] == 0) return true;
  }
  return false;
    ...
    slide([
      title('Dumb Code Stub'),
      codeBlock(`;; Computing this.primes
...
mov eax,[ebx, <primes offset>]`)
    ...
    slide([
      title('Turbofan'),
      lineEm('The optimizing compiler'),
      line('Kicks in when the code is hot'),
      line('Uses the type info collected by ICs'),
      line('Optimizes like there\'s no tomorrow')
    ]),

    slide([
      title('Turbofan Pipeline', false, 6),
      image(images.v8turbofanPipeline, '80vw')
    ]),
...
idiv esi`, '0.5em')
    ...
    slide([
      ...
      line('throws away optimized code'),
      line('resumes execution at the right place in ignition'),

...
fn validation_service(
    order: Order,
) -> Pin<Box<dyn Future<Output = Result<ValidatedOrder, OrderNotValid>>>> {
...
pub struct FpCheckedProcessor {}

impl AsyncProcessor for FpCheckedProcessor {
...
impl FpCheckedProcessor {
    pub fn processor() -> &'static dyn AsyncProcessor {
        &(FpCheckedProcessor {}) as &dyn AsyncProcessor
    }
}

pub fn process_fp_checked_direct(key: usize) -> impl Future<Output = ProcessResult> {
